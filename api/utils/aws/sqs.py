...
def send_messages_by_url(
    ...
    try:
        ...
        logging.exception("Failed to send sqs message")
        raise err
...
def send_message_batch_by_url(queue_url: str, entries: list):
    ...
    Raises:
        ...
        RuntimeError: If an unexpected error is founded
    """
    logging.info(f"Data executed: Queue URL: -> {queue_url}, Entries to send: {entries}")
    try:
        sqs_client = get_sqs_client()
        ...
        logging.exception("Failed to send message batch to sqs")
        raise err
    except Exception as err:
        logging.exception("Unexpected error to try to send message batch")
        raise err
...
def receive_message(queue_url, max_number_messages, wait_time):
    ...
    try:
        ...
        logging.exception("Failed to receive messages to the current queue url")
        raise err
    except Exception as err:
        logging.exception("Unexpected error was found when try to receive messages")
        raise err
